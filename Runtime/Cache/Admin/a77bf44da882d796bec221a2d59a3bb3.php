...
layer.config({
    extend: [
      ...
      <div class="x12 padding-top padding border-bottom">
		<h3 class="x3">角色管理</h3>
		<button class="button float-right bg-sub icon-plus" type="button"  onclick="role_add(this)" > 创建角色</button>
      </div>
      <div class="x12">
		<div class="x12 padding">
			<ul class="list-group list-striped radius-none">
				<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><li><span class="float-right"><a href="javascript:void(0)" onclick="role_edit(<?php echo ($v["id"]); ?>)"  class="text-blue">分配权限</a> </span><?php echo ($v["title"]); ?></li><?php endforeach; endif; else: echo "" ;endif; ?>
			</ul>
		</div>
      </div>
    </div>
	<script>
		...
		function role_add(){
			layer.open({
				...
				maxmin: true,
				title:'创建角色',
				content: '/index.php?s=/Admin/Auth/role_add'
			});
		}
		//编辑角色
		function role_edit(id){
			layer.open({
				...
				maxmin: true,
				title:'编辑角色',
				content: '/index.php?s=/Admin/Auth/role_edit/id/'+id
			});
		}
