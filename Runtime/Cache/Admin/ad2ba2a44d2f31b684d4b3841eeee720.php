...
<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$zml): $mod = ($i % 2 );++$i;?><div class="x12 bg padding">
		...
		<div class="x9 bg padding bg-white border-left height-big">
			...
			<button class="button radius-none bg-red"  onclick="iframe('<?php echo U('Admin/Item/item_editstatus',array('itemid'=>$zml['id']));?>')" type="button"><?php echo ($zml["progress_name"]); ?></button>
			<button onClick="iframe('<?php echo U('Admin/Item/item_video',array('itemid'=>$zml['id']));?>')" class="button radius-none bg-sub margin-left">视频管理</button>
			...
			<div class="x12">
				...
				<a href="javascript:void(0)" onclick="delitem('<?php echo U('Admin/Item/item_del',array('id'=>$zml['id'],'isdel'=>1));?>')" class="radius-none button button-small bg-sub">删除</a>
				...
				<?php if($zml['status'] == '0'): ?><span class="badge bg-yellow-light">未审核</span><?php endif; ?>
			</div>
