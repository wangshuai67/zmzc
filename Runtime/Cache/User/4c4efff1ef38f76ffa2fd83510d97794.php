    ...
    <div class="x12 padding-large-top" style="background:url(/Template/User/default/Public/img/register_bj.jpg) center;height:500px;"  >
		<div class="container">
			<div class="x4 float-right bg-white padding-large">
				...
				<a href="javascript:void(0)"  class="x12 bg-red   text-center padding text-white text-large" onclick="login(this)">立 即 注 册</a>
				<div class="x12 margin-top">
					<span>已有账号  【<a href="/index.php?s=/User/Login/index" class="text-blue">立即登录</a>】</span>
				</div>
			</div>
		</div>
	</div>
	<script>
		...
		function getyzm(d){			
			...
			if(telReg == false){
				...
				$.post('/index.php?s=/User/Login/sendsms.html',{phone:phone.val()},function(e){
					if(e.status==1){
						layer.msg(e.info);
						time(d)
						$(d).text('验证已发送');
		...
		function verify(id){
			$("#"+id).attr('src',"<?php echo U('Home/Verify/index');?>");
		}
		function login(d){
			var phone=$("input[name='phone']")
